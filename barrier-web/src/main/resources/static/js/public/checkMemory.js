...
{
	if (checked) addCheckedArray(data.id,data.id);
	else removeCheckedArray(data.id,data.id);
}
function checkAllRowHandler(checked)
{
	...
	{
		if(checked)
			addCheckedArray(this.records[rowid]['id'],this.records[rowid]['id']);
		else
			removeCheckedArray(this.records[rowid]['id'],this.records[rowid]['id']);
	}
}