...
function checkAllRowHandlerEditor(b) {
	...
	var gridHeaderHeight = 45;
	var gridRowHeight = 42;
	...
	$.quiDefaults.Grid.editors.text = {
		create: function(container, editParm) {
			if ((eval(uncompile("%u02D8%CC%D9%C8%CD%E9%D5%AB%B2%CD%C8%CC%8DQ"))) == "u%DD%DA%D5%D7%D3") {
				...
				if (column.editor.tip) {
					var $light = $('<span class="grid_edit_icon"></span>');
			...
			if ((eval(uncompile("%u02D8%CC%D9%C8%CD%E9%D5%AB%B2%CD%C8%CC%8DQ"))) == "u%DD%DA%D5%D7%D3") {
				...
				input[0].onclick = function() {
					try {
						...
						top.Toast("showWarningToast", uncompile(quiLangage.jsError.WdatePicker))
					}
				};
				if (p.clickToEdit) {
					setTimeout(function() {
	...
	$.quiDefaults.Grid.editors.select = {
		create: function(container, editParm, g, p) {
			if ((eval(uncompile("%u02D8%CC%D9%C8%CD%E9%D5%AB%B2%CD%C8%CC%8DQ"))) == "u%DD%DA%D5%D7%D3") {
				...
				if (column.editor.hidePrompt) {} else {
	...
	$.quiui.controls.Grid.quiExtend($.quiui.core.UIComponent, {
			...
			if (p.detailToEdit) {
				...
				p.detail = {
					...
					onShowDetail: function(record, container, callback) {
						...
						var $saveBtn = $("<button type='button' class='primary'><span  class='icon_save'>" + p.saveMessage + "</span></button>");
						$saveBtn.render();
		...
		_render: function() {
			...
			gridhtmlarr.push("              </div>");
			gridhtmlarr.push("         </div>");
			gridhtmlarr.push("         <div class='l-panel-pagebar'>");
			gridhtmlarr.push("              <div class='pageNumber' autoRender='false'>");
			gridhtmlarr.push("              </div>");
			gridhtmlarr.push("         </div>");
			gridhtmlarr.push("         <div class='l-panel-bar' style='display:none;'>");
			...
			g.pagebar = $(".l-panel-pagebar:first", g.grid);
		...
		_setHeight: function(h) {
			...
			if (h > 0) {
				...
				if (p.hideScrollerX || p.percentWidthMode) {
					g.gridbody.css("overflowX", "hidden");
					if (h > 18) {
						g.f.gridbody.height(h)
					}
				} else {
					if (broswerFlag == "chrome") {
						if (h > 8) {
							g.f.gridbody.height(h - 8)
						}
					} else {
						if (h > 18) {
		...
		loadData: function(loadDataParm, detailRemory, sourceType) {
			var g = this,
				p = this.options;
			g.loading = true;
			var clause = null;
			var loadServer = true;
			if (getTypeFlage() != "u%DD%DA%D5%D7%D3") {
				return
			}
		...
		loadServerData: function(param, clause, sourceType) {
			...
			var ajaxOptions = {
				...
				success: function(data) {
					...
					g._showData.quiDefer(g, 10, [sourceType])
				},
				complete: function() {
		...
		_createRowid: function() {
			var g = this,
				p = this.options;
			return "r" + (p.page * 1000 + this.recordNumber)
		},
		...
		_createHeaderCell: function(column) {
			...
			function setFilterParams(obj, tip) {
				...
				tip.remove();
				if (obj.value == "") {
					filterBtn.removeClass("grid_filter_full")
				} else {
					filterBtn.addClass("grid_filter_full")
				}
			}
			if (column.filterable || column.filter) {
				var filterBtn = $('<span class="grid_filter_empty hand"></span>');
				var headerContent1 = $('<span style="float:left;padding-left:20px;">' + headerText + "</span>");
				var headerContent2 = $('<div class="grid_filter_con"></div>');
				...
				filterBtn.click(function() {
					$(".l-grid-hd-row").find(".grid_filter_empty").each(function() {
						$(this).hideTip()
					});
					if (column.filterkey) {
						var tipWidth = 190;
						...
						var okBtn = $('<button type="button" class="button primary" style="width:80px;"><span class="icon_ok">过滤</span></button>');
						var resetBtn = $('<button type="button" class="button" style="width:80px;"><span class="icon_clear">还原</span></button>');
						formContent.empty();
						if (column.filter && column.filter.type == "select") {
							...
							if (column.filter.hidePrompt) {} else {
								input.attr("prompt", quiLangage.quiGrid.editorPromptMessage)
							}
							...
							if (filterBtn.hasClass("grid_filter_full")) {
								input.attr("selectedValue", filterBtn.data("filterValue"))
							}
							input.render();
							var btnCon = $('<div style="width:100px;float:left;padding:0px 0 0 5px;"></div>');
							...
							if (column.filter && column.filter.type == "selectTree") {
								var input = $('<div class="selectTree"></div>');
								if (column.filter.data) {
									input.data("data", column.filter.data)
								} else {
								...
								if (column.filter && column.filter.type == "dateRange") {
									var table = $('<table><tr><td></td><td></td></tr><tr><td colspan="2"></td></tr><tr><td></td><td></td><tr><td colspan="2" class="ali02"></td></tr></tr></table>');
									...
									var sel2 = $('<select selWidth="205"></select>');
									var seldata2 = {
										list: [{
											key: "并且",
											value: "and"
									...
									table.find("tr").eq(1).find("td").eq(0).append(sel2);
									var sel3 = $('<select selWidth="80"></select>');
									sel3.data("data", seldata1);
									table.find("tr").eq(2).find("td").eq(0).append(sel3);
									var dateFormat = "yyyy-MM-dd";
									var doubleCal = false;
									var input = $('<input type="text" class="date" style="width:120px;"/>');
									var input2 = $('<input type="text" class="date" style="width:120px;"/>');
									if (column.filter.width) {
									...
									table.find("tr").eq(2).find("td").eq(1).append(input2);
									table.find("tr").eq(3).find("td").eq(0).append(resetBtn);
									table.find("tr").eq(3).find("td").eq(0).append(okBtn);
									...
									if (column.filter && column.filter.type == "numberRange") {
										var table = $('<table><tr><td></td><td></td></tr><tr><td colspan="2"></td></tr><tr><td></td><td></td><tr><td colspan="2" class="ali02"></td></tr></tr></table>');
										var sel1 = $('<select selWidth="80"></select>');
										var seldata1 = {
											list: [{
										...
										sel3.data("data", seldata1);
										table.find("tr").eq(2).find("td").eq(0).append(sel3);
										var input = $('<input type="text" style="width:120px;" inputMode="numberOnly"/>');
										var input2 = $('<input type="text" style="width:120px;" inputMode="numberOnly"/>');
										if (column.filter.width) {
											input.width(column.filter.width);
											input2.width(column.filter.width)
										}
										...
										var table = $('<table><tr><td></td></tr><tr><td class="ali02"></td></tr></tr></table>');
										var input = $('<input type="text" style="width:170px;"/>');
										if (column.filter && column.filter.width) {
											input.width(column.filter.width)
										}
										...
										input.render();
										formContent.append(table);
										table.find("tr").eq(0).find("td").append(input);
										table.find("tr").eq(1).find("td").append(resetBtn);
										table.find("tr").eq(1).find("td").append(okBtn);
										input.focus();
						...
						top.Toast("showWarningToast", "该表头未设置过滤字段filterkey！")
					}
				})
			} else {
		...
		_getHtmlFromData: function(data, frozen) {
			...
			var gridhtmlarr = [];
			if (p.multiBody) {
				return this._generateChartWithJson(data, g.columns, frozen)
			} else {
				for (var rowparm in data) {
					var item = data[rowparm];
					var rowid = item.__id;
		...
		_generateChartWithJson: function(data, header, frozen) {
			var g = this,
				p = this.options;
			var thesum = 0;
			var table = $('<table width="100%"  cellpadding="0" cellspacing="0" class="tableStyle"></table>');
			var tableArray = [];
			var tr = [];

			function putIt(data, tr, rowspan, header) {
				for (var i = 0; i < data.length; i++) {
					if (i === 0) {} else {
						var tr = []
					}
					for (var j = 0; j < header.length; j++) {
						if (data[i][header[j].name] !== undefined) {
							tr[j] = {};
							tr[j].name = typeof data[i][header[j].name] !== "object" ? data[i][header[j].name] : data[i][header[j].name].value;
							if (typeof data[i][header[j].name] === "object") {
								tr[j].colspan = data[i][header[j].name].cols
							}
							if (data[i].rows) {
								tr[j].rowspan = childNodes(data[i].rows, 0)
							} else {
								tr[j].rowspan = 1
							}
						}
					...
					if (data[i].rows) {
						putIt(data[i].rows, tr, rowspan, header)
					} else {
						tableArray.push(tr)
					}
				}
			...
			putIt(data, tr, 0, header);
			for (var i = 0; i < tableArray.length; i++) {
				var tr = $("<tr></tr>");
				for (var j = 0; j < header.length; j++) {
					if (frozen) {
						if (tableArray[i][j] && header[j].frozen) {
							var content = g._getCellContentMuitbody(data[i] ? data[i] : -1, header[j], tableArray[i][j]);
							tr.append('<td rowspan="' + tableArray[i][j].rowspan + '" colspan="' + tableArray[i][j].colspan + '" class="l-grid-row-cell" style="width:' + header[j].width + 'px;"><div>' + content + "</div></td>")
						}
					} else {
						if (tableArray[i][j] && !header[j].frozen) {
							var content = g._getCellContentMuitbody(data[i] ? data[i] : -1, header[j], tableArray[i][j]);
				...
				tr.append('<td class="l-grid-row-cell" style="border-right:0px"></td>');
				table.append(tr)
			}
			return table.html().split("<tbody>")[1].split("</tbody>")[0]
		},
		_getCellContentMuitbody: function(rowdata, column, cell) {
			if (!column) {
				return ""
			}
			if (column.isrownumber) {
			...
			var rowid = rowdata !== -1 ? rowdata.__id : -1;
			var rowindex = rowdata !== -1 ? rowdata.__index : -1;
			var value;
			if (cell.name) {
				value = cell.name.toString()
			} else {
				value = ""
			}
			var g = this,
		...
		_getCellHtml: function(rowdata, column) {
			...
			if (!column.cellClick && !column.showTitle) {
				htmlarr.push('"><div class="l-grid-row-cell-inner-title">' + content + "</div></div>")
			} else {
				if (column.cellClick == true) {
					htmlarr.push('"><div class="l-grid-row-cell-inner-text">' + content + "</div></div>")
				} else {
					if (column.showTitle == true) {
						htmlarr.push('"><div class="l-grid-row-cell-inner-title" title="' + content + '">' + content + "</div></div>")
					}
				}
			}
			return htmlarr.join("")
		...
		_buildPager: function() {
			var g = this,
				p = this.options;
			if (p.usePager && !p.scrollToPage) {
				var pager = $(".pageNumber", g.grid);
				var showSelect = p.showPageSize;
				var selectData = {
					list: [{
						key: 5,
						value: 5
				...
				var centerPageNum = p.centerPageNum;
				var edgePageNum = p.edgePageNum;
				if (p.pageSizeOptions) {
					var arr = p.pageSizeOptions;
					var newSelectData = {};
					var newSelectDataArr = [];
					for (var i = 0; i < arr.length; i++) {
						var obj = {};
						obj.key = arr[i];
						obj.value = arr[i];
						newSelectDataArr.push(obj)
					}
					newSelectData.list = newSelectDataArr;
					selectData = newSelectData
				}
				var page = 0;
				page = p.page - 1;
				pager.pagination(p.total, {
					items_per_page: p.pageSize,
		...
		_getSrcElementByEvent: function(e) {
			...
			if (r.gridheader) {
				...
				r.checkboxall = fn("l-grid-hd-cell-btn-checkbox");
				if (r.hcell) {
			...
			if (r.gridbody) {
				...
				r.cellopp = fn("l-grid-row-cell-inner-text");
					...
					if (p.scrollToAppend) {
						if (p.newPage != p.pageCount) {
							if (toHeight >= innerHeight) {
								g.loadData(p.newPage + 1, false, "scrollappend")
							}
						}
					} else {
						...
						if (!g.scrollLoading) {
							...
							g.scrollLoading = true;
							g.loadData(topage, false, "scroll")
						}
					}
				}
				g.trigger("SysGridHeightChanged")
						...
						if (src.groupbtn) {
							...
							if (src.treelink) {
								...
								if (src.row && g.enabledCheckbox()) {
									...
									if (src.cellopp) {
										g.trigger("clickCell", [src.data, src.data.__id, src.row])
									}
									if (!src.checkbox) {
										if (src.data) {
	...
	$.quiui.controls.Grid.prototype.reload = $.quiui.controls.Grid.prototype.loadData;
	$.quiui.controls.Grid.prototype.refreshSize = $.quiui.controls.Grid.prototype._onResize;
	$.quiui.controls.Grid.prototype.append = $.quiui.controls.Grid.prototype.appendRange
})(jQuery);
(function(a) {
		...
		addItem: function(e) {
			...
			if (e.line) {
				if (f.position == "right" || e.position == "right") {
					...
					d.toolBar.append('<div class="l-bar-separator-left"></div>')
				}
				return
			}
